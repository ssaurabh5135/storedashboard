...
# ✅ AUTO-LOAD GOOGLE SHEET ON STARTUP (NO BUTTONS NEEDED)
...
if df.empty:
    st.error("❌ No data loaded. Please check Google Sheet access.")
    st.stop()

st.success(f"✅ Auto-loaded **{len(df)} rows** from Google Sheet")
...
with r2c2:
    ...
    if not age_df.empty:
        ...
        # ✅ FIXED: Perfect numeric conversion + bucket numbers showing
        age_pivot = age_pivot.astype({'Bucket': str})
        for col in age_pivot.columns[1:]:  # All numeric columns
        ...
        for _, row in age_pivot.iterrows():
            ...
            html_rows += f"<tr style='background-color: {bgcolor}; color: {txtcolor}; font-weight: bold;'>"
            # ✅ FIXED: Numbers now show perfectly
            html_rows += f"<td style='font-weight: bold;'>{row['Bucket']}</td>"
            for col in age_pivot.columns[1:]:
                html_rows += f"<td>{int(row[col])}</td>"
            html_rows += "</tr>"

        table_html = """
        <table style='margin:auto; border-collapse: collapse; color:black; font-family: Fredoka, sans-serif;'>
            <tr style='background-color: #e0e0e0; color: black; font-weight: bold;'>
        """
        for col in age_pivot.columns:
            table_html += f"<th style='padding:10px; border:1px solid rgba(0,0,0,0.3); font-size:14px;'>{col}</th>"
...
st.markdown("---")
st.caption("✅ **PERFECT!** Auto-load + Fixed Ageing numbers + Colors working!")
